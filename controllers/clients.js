...
const renderCartPage = (req, res, next) => {
  ...
  let user = req.user;
  user
    .populate("cart.item._id", "title image price")
    .execPopulate()
    .then((user) => {
      user.cart.forEach((item) => total += (item.item._id.price * item.item.qty));
      return total
    }).then((total) => {
      console.log(total)
...
const renderHomePage = (req, res, next) => {
  ...
  const userName = localStorage.getItem("userName");
  return Product.countDocuments((err, total) => {
    if (err) {
      throw new Error("Total items number not found");
    }
    return total;
  })
    .then((total) => {
    ...
    .catch((err) => console.log("Error caught rendering Home Page", err));
};
